{ ... }:
{
  flake.types.generic.feature-options.zsh =
    { inputs, lib }:
    let
      inherit (inputs.self.types.generic) simpleFeature;
    in
    simpleFeature { inherit inputs lib; } "zsh feature";
  ...
  flake.modules.homeManager.zsh =
    ...
    }:
    {
      config =
        ...
        in
        lib.mkIf config.host.features.zsh.enable {
  ...
  flake.modules.nixos.zsh-feature =
    {
      config,
      lib,
      pkgs,
