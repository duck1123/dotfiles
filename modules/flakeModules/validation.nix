...
let
  ...
  validateFeatureDependencies = host: let
    errors = [];
    ...
    syncthingErrors = if host.features.syncthing.enable then
      let
        ...
        hasShares = shares.camera.enable || shares.keepass.enable || shares.org-roam.enable || shares.renpy.enable;
      in
      if !hasShares then
    ...
    kubernetesErrors = 
      ...
      (if host.features.kubernetes.server.enable && !host.features.kubernetes.client.enable then
      ...
      (if host.features.kubernetes.client.enable && !host.features.developer.enable then
    ...
    nixosErrors = if host.nixos.enable then
      ...
      (let
        hasDesktopFeatures = host.features.hyprland.enable || host.features.gnome.enable || host.features.i3.enable;
        hasDesktopNixos = host.nixos.hyprland.enable || host.nixos.gnome.enable || host.nixos.plasma6.enable || host.nixos.budgie.enable || host.nixos.i3.enable;
      in
      if hasDesktopFeatures && !hasDesktopNixos then
        [ "Host ${host.name}: desktop features are enabled but no desktop environment is selected in NixOS configuration" ]
      else [])
    else [];
    
    allErrors = errors ++ desktopErrors ++ syncthingErrors ++ kubernetesErrors ++ devErrors ++ mediaErrors ++ nixosErrors;
  in
  if allErrors != [] then
    throw "Host validation errors:\n" + builtins.concatStringsSep "\n" allErrors
  else
    host;

  # Generate warnings for potential issues
  generateWarnings = host: let
    warnings = [];
    ...
    desktopWarnings = 
      ...
      (if host.features.stylix.enable && host.nixos.enable then
        let
          hasDesktop = host.nixos.hyprland.enable || host.nixos.gnome.enable || host.nixos.plasma6.enable || host.nixos.budgie.enable || host.nixos.i3.enable;
        in
        if !hasDesktop then
  ...
  validateHosts = hosts: builtins.mapAttrs (name: host: validateHost host) hosts;

in {
  inherit validateHost validateHosts generateWarnings;
}
