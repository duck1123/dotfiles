...
let
  ...
  checkFeatureConsistency = hosts: let
    ...
    hostFeatures = builtins.mapAttrs (name: host: {
      ...
      features = builtins.filter (feature: feature.enable) (
        builtins.attrValues (
          builtins.mapAttrs (featureName: feature: 
            if builtins.isAttrs feature && builtins.hasAttr "enable" feature then
              { name = featureName; enable = feature.enable; }
            else
              { name = featureName; enable = false; }
          ) host.features
        )
      );
    }) hosts;
    ...
    inconsistencies = builtins.concatLists (
      builtins.attrValues (
        builtins.mapAttrs (name: hostInfo: 
          builtins.map (feature: 
            "Host ${hostInfo.name}: ${feature.name} is enabled"
          ) hostInfo.features
        ) hostFeatures
      )
    );
  ...
  validateHostsComprehensive = hosts: let
