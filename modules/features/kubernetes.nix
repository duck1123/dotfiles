  ...
  flake.modules.nixos.kubernetes-feature =
    ...
    }:
    {
      config = lib.mkMerge [
        ...
        # which forces tokenPath even when server.enable is false. Keeping this secret
        # available regardless of server.enable means that forced read never hits a missing
        # attribute, so server.enable can be toggled off without also having to clear tokenFile.
        {
